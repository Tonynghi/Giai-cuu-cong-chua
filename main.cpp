...
int MG3(int arr[100], int size){
    ...
    for(int c=0;c<size;c++){
        if(arr[c]<0) arr2[c]=0-arr[c];
        arr2[c]=(17*arr[c]+9)%257;
    }
    int max= arr2[0];
    int maxi=0;
    int min = arr2[0];
...
int main() {
    string filena ="test.txt";
    int HP, level, remedy, maidenkiss, phoenixdown, rescue;
    adventureToKoopa(filena, HP, level, remedy, maidenkiss, phoenixdown, rescue);
